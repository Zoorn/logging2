...
def test_load_config(
    mock_importlib_open, mock_exists, mock_open, mock_makedirs, logger_instance
):
    ...
    logger_instance.load_config(
        "logging_file", log_file_path=log_file_path, log_level="CRITICAL"
    )
...
def test_get_logger(logger_instance):
    ...
    ), "Returned object is not a logger instance"
...
def test_load_config_package(manage_virtualenv):
    ...
    assert isinstance(
        bub_logger, BubLogger
    ), "Returned object is not a BubLogger instance"
    assert bub_logger.configs, "Config was not loaded"
    assert bub_logger.configured, "Logger was not configured"
